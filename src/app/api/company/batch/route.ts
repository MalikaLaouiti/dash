...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`📥 Received ${companies?.length || 0} companies to insert`);
    ...
    console.log('📄 First companie data:', JSON.stringify(companies[0], null, 2));
    ...
    for (let i = 0; i < companies.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        if (insertedIds.length > 0) {
          ...
          if (found) {
            console.log(`   Found doc:`, { _id: found._id, nom: found.nom });
          }
        }
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('📝 GET - Collection:', Company.collection.name);
